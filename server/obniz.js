...
class GetGpsPosition {
    ...
    getGpsData() {
        ...
        var longitude = 0,
            latitude = 0;
        let d = this.gps.getEditedData();
        if (d.enable) {
            if (d.GPGGA) {
                let p = d.GPGGA;
                if (p[6] != "0") {
                    //�o�x
                    longitude = this.gps.nmea2dd(p[4]);
                    //�ܓx
                    latitude = this.gps.nmea2dd(p[2]);
                }
            }
        }
    ...
    getXY() {
        let pos = this.getGpsData(this.gps);

        var gpsStatus = false;
        if (pos.latitude) gpsStatus = true;

        let rad_latitude = pos.latitude * Math.PI / 180;
        let rad_longitude = pos.longitude * Math.PI / 180;

        const R = 6378150;//m
        var x = R * rad_latitude;
        var y = R * Math.cos(rad_latitude) * rad_longitude;

        console.log("lati: " + pos.latitude + " \tlongi: " + pos.longitude);
        //console.log("cos: "+Math.cos(rad_latitude *Math.PI /180));

        return { "x": x, "y": y, "gpsStatus": gpsStatus };
    };
...
let baseObniz = new Obniz("6379-9359");

baseObniz.onconnect = async function () {
    Obniz.PartsRegistrate(DPS310);
    baseObniz.setVccGnd(11, 9, '5v');
    let sensor = baseObniz.wired("DPS310", { sda: 0, scl: 1, gnd: 2 });
    let baseGps = baseObniz.wired("GYSFDMAXB", { vcc: 8, gnd: 7, txd: 6, rxd: 5, Opps: 4 });

    await sensor.init();

    while (1) {
        let pressure = await sensor.measurePressureOnce();
        let getGpsPosition = new GetGpsPosition(baseGps);
        let gpsPos = getGpsPosition.getXY();

        let pOptions = {
            method: "POST",
            uri: "http://localhost:2828/pressure",
            json: {
                "device": "base",
                "pressure": pressure
            }
        };
        let pResponse = await request.post(pOptions);
        let gOptions = {
            method: "POST",
            uri: "http://localhost:2828/gps",
            json: {
                "device": "base",
                "gps": { x: gpsPos.x, y: gpsPos.y }
            }
        }
        let gResponse = await request.post(gOptions);
        console.log("base: p:" + pressure + ", x:" + gpsPos.x + ", y:" + gpsPos.y);
        console.log("base: pressure: " + pResponse.status + " gps: " + gResponse.status);
        await baseObniz.display.clear();
        await baseObniz.display.print("base");
        await baseObniz.display.print("\np: " + Math.floor(pressure));
        await baseObniz.display.print("\nx: " + Math.floor(gpsPos.x));
        await baseObniz.display.print("\ny: " + Math.floor(gpsPos.y));
        await baseObniz.wait(5000);
    };
}


let childObniz = new Obniz("6379-9359");

childObniz.onconnect = async function () {
    Obniz.PartsRegistrate(DPS310);
    childObniz.setVccGnd(11, 9, '5v');
    let sensor = childObniz.wired("DPS310", { sda: 0, scl: 1, gnd: 2 });
    let childGps = childObniz.wired("GYSFDMAXB", { vcc: 8, gnd: 7, txd: 6, rxd: 5, Opps: 4 });

    await sensor.init();

    while (1) {
        let pressure = await sensor.measurePressureOnce();
        let getGpsPosition = new GetGpsPosition(childGps);
        let gpsPos = getGpsPosition.getXY();
        ...
        let gResponse = await request.post(gOptions);
        console.log("child: p:" + pressure + ", x:" + gpsPos.x + ", y:" + gpsPos.y);
        console.log("child: pressure: " + pResponse.status + " gps: " + gResponse.status);
        await childObniz.display.clear();
        await childObniz.display.print("child");
        await childObniz.display.print("\np: " + Math.floor(pressure));
        await childObniz.display.print("\nx: " + Math.floor(gpsPos.x));
        await childObniz.display.print("\ny: " + Math.floor(gpsPos.y));
        await childObniz.wait(5000);
    };
}

let parentObniz = new Obniz("8492-9425");

parentObniz.onconnect = async function () {
    Obniz.PartsRegistrate(DPS310);
    parentObniz.setVccGnd(11, 9, '5v');
    let sensor = parentObniz.wired("DPS310", { sda: 0, scl: 1, gnd: 2 });
    let parentGps = parentObniz.wired("GYSFDMAXB", { vcc: 8, gnd: 7, txd: 6, rxd: 5, Opps: 4 });

    await sensor.init();

    while (1) {
        let pressure = await sensor.measurePressureOnce();
        let getGpsPosition = new GetGpsPosition(parentGps);
        let gpsPos = getGpsPosition.getXY();
        ...
        let gResponse = await request.post(gOptions);
        console.log("parent: p:" + pressure + ", x:" + gpsPos.x + ", y:" + gpsPos.y);
        console.log("parent: pressure: " + pResponse.status + " gps: " + gResponse.status);
        await parentObniz.display.clear();
        await parentObniz.display.print("parent");
        await parentObniz.display.print("\np: " + Math.floor(pressure));
        await parentObniz.display.print("\nx: " + Math.floor(gpsPos.x));
        await parentObniz.display.print("\ny: " + Math.floor(gpsPos.y));
        await parentObniz.wait(5000);
    };
}
